...
- `PasteGlideShared` contient maintenant les modeles, la classification, la recherche/filtrage, les positions de panneau, les regles de preferences et un format d'archive JSON versionne sans AppKit, ce qui donne une premiere base compilable pour les ports.
...
1. Continuer l'extraction de la base SQLite dans `PasteGlideShared`; l'import/export JSON dispose deja d'un format portable partage.
...
Le workflow GitHub Actions `Portable Packages` construit ce binaire portable en environnement natif Linux et Windows, et reconstruit aussi les paquets macOS. Il publie des artefacts de workflow:

- macOS: `.dmg` et `.app.zip`
- Linux: `.deb` et `.rpm`
- Windows: `.zip` et `.msi`

Quand le workflow est declenche par un tag `v*`, ces paquets sont aussi attaches a la release GitHub `PasteGlide` avec des notes FR/EN, sans date ni heure dans le nom de release.
...
- `PasteGlideShared` now contains the models, classification, search/filtering, panel positions, preference rules, and a versioned JSON archive format without AppKit, providing a first compilable base for the ports.
...
1. Continue extracting SQLite storage into `PasteGlideShared`; JSON import/export already has a shared portable format.
...
The `Portable Packages` GitHub Actions workflow builds this portable binary in native Linux and Windows environments, and also rebuilds the macOS packages. It publishes workflow artifacts:

- macOS: `.dmg` and `.app.zip`
- Linux: `.deb` and `.rpm`
- Windows: `.zip` and `.msi`

When the workflow is triggered by a `v*` tag, these packages are also attached to the `PasteGlide` GitHub release with FR/EN notes, without a date or time in the release name.
